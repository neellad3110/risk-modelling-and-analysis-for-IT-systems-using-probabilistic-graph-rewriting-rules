...
def build_ctmc_from_lts(lts: LTSGraph, rule_specs: Dict[str, RuleSpec]) -> Tuple[CTMC, str]:
    ...
    # Precompute per-state, per-rule counts of enabled parent edges (matches)
    # This supports campaign-rate splitting: for a state s and rule r,
    # K(s,r) = number of parent edges labeled with that rule from s.
    parent_count_by_state_rule: Dict[Tuple[str, str], int] = {}
    for src, outs in edges_by_src.items():
        # Count only parent attack edges
        tmp: Dict[str, int] = {}
        for e in outs:
            fn = _extract_fn_name(e.label)
            rid = parent_map_default.get(_norm(fn))
            if rid:
                tmp[rid] = tmp.get(rid, 0) + 1
        for rid, k in tmp.items():
            parent_count_by_state_rule[(src, rid)] = k
    ...
    for e1 in lts.edges:
        ...
        if not spec or spec.lam <= 0.0:
            continue
        # Campaign-rate splitting: divide by number of enabled matches for (src,rid)
        K = max(1, parent_count_by_state_rule.get((e1.src, rid), 1))
        lam_eff = float(spec.lam) / float(K)
    ...
    if all(len(v) == 0 for v in ctmc.values()):
        # Build per-state outcome-edge counts by inferred rule
        outcome_count_by_state_rule: Dict[Tuple[str, str], int] = {}
        for src, outs in edges_by_src.items():
            counts: Dict[str, int] = {}
            for e in outs:
                fn = _extract_fn_name(e.label)
                norm = _norm(fn)
                # Map outcome label to rule id
                rid_match: Optional[str] = None
                for rid, ocmap in outcome_idx_by_rule.items():
                    if norm in ocmap:
                        rid_match = rid
                        break
                if rid_match:
                    counts[rid_match] = counts.get(rid_match, 0) + 1
            for rid, k in counts.items():
                outcome_count_by_state_rule[(src, rid)] = k
        ...
        for e in lts.edges:
            ...
            for rid, ocmap in outcome_idx_by_rule.items():
                ...
                if not spec or spec.lam <= 0.0:
                    continue
                # Split campaign rate across outcome-labeled edges for this rule at this state
                K = max(1, outcome_count_by_state_rule.get((e.src, rid), 1))
                lam_eff = float(spec.lam) / float(K)
                q = lam_eff * float(oc.prob)
                if q <= 0:
                    continue
