...
def write_prism_ctmc(out_model: Path, S: List[str], trans, s_init_idx: int):
    ...
    lines.append('rewards "events"\n')
    for i, (_, _, _, _, rid, _, _) in enumerate(trans):
        lines.append(f"  [{rid}_{i}] true : 1;\n")
    lines.append("endrewards\n\n")

    # Per-rule rewards
    from collections import defaultdict
    idxs_by_rule = defaultdict(list)
    for i, (_, _, _, _, rid, _, _) in enumerate(trans):
        idxs_by_rule[rid].append(i)

    for rid, idxs in idxs_by_rule.items():
        # Per-rule cost
        lines.append(f'rewards "cost_{rid}"\n')
        for i in idxs:
            cost = trans[i][3]
            if abs(cost) > 0:
                lines.append(f"  [{rid}_{i}] true : {cost};\n")
        lines.append("endrewards\n\n")
        # Per-rule number of events
        lines.append(f'rewards "events_{rid}"\n')
        for i in idxs:
            lines.append(f"  [{rid}_{i}] true : 1;\n")
        lines.append("endrewards\n\n")
