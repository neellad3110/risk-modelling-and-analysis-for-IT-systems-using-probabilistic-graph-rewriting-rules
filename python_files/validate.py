...
def get_typed_graph_info(typed_graph):
    ...
    relation_types = set()
    node_attributes = {}
    # Only use schema definitions (those with "name")
...
def main():
    ...
    node_types, rel_types, node_attrs = get_typed_graph_info(typed_graph)

    errors = []
    errors += check_nodes_and_attrs(business_graph["business_graph"], node_types, node_attrs, "Business Graph")
    errors += check_relations(business_graph["business_graph"], rel_types, "Business Graph")
    errors += check_rules(risk_rules, node_types, rel_types, node_attrs)

    if errors:
        print("Consistency check failed:")
        for err in errors:
            print(" -", err)
        print(f"\nTotal inconsistencies found: {len(errors)}")
    else:
        print("All graphs and rules are consistent!")
